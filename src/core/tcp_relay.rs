...
use crate::core::{config::ServerConfig, copy::pipe, socks5::Socks5Address, Context};

/// Server is a server block
/// {
///      "listen": 1080,
///      "password": "xxx"
/// }
...
impl Server {
    pub fn new(config: ServerConfig, context: Arc<Context>) -> Server {
    ...
    pub async fn run_server(self) -> io::Result<()> {
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(err) = handler.serve().await {
...
impl TcpServerConnectionHandler {
    pub async fn serve(mut self) -> io::Result<()> {
        ...
        match future::select(fut1, fut2).await {
            ...
            Either::Left((Err(err), ..)) => {
                debug!(
                    "connection error on {} to {} with error {}",
                    &address, &self.remote_addr_in_local, err
                );
            }
            Either::Right((Ok(..), ..)) => {}
            Either::Right((Err(err), ..)) => {
                debug!(
                    "connection error on {} to {} with error {}",
                    &self.remote_addr_in_local, &address, err
                );
            }
        }
        Ok(())
    ...
    pub async fn connect_remote(ctx: &Context, address: &Socks5Address) -> io::Result<TcpStream> {
        let addr = match *address {
            Socks5Address::Domain(ref domain, ref port) => Self::lookup_domain(&domain).await?,
